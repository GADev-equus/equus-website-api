...
const analyticsController = {
  ...
  async getOverview(req, res) {
    try {
      ...
      const uniqueVisitorsCount = Array.isArray(uniqueVisitors) ? uniqueVisitors.length : uniqueVisitors;
    ...
    try {
      ...
      const [
        ...
        Analytics.aggregate([
          ...
          {
            $group: {
              _id: { $cond: [{ $ne: ['$userId', null] }, 'authenticated', 'anonymous'] },
              count: { $sum: 1 },
      ...
      const performanceData = await Analytics.aggregate([
        ...
        },
        {
          $project: {
            ...
            errorCount: 1,
            errorRate: { 
              $round: [{ $multiply: [{ $divide: ['$errorCount', '$totalViews'] }, 100] }, 2] 
            }
          }
        },
        { $sort: { [sortBy]: -1 } },
        ...
        },
        {
          $project: {
            ...
            slowRequestsPercentage: { 
              $round: [{ $multiply: [{ $divide: ['$slowRequests', '$totalRequests'] }, 100] }, 2] 
            },
            errorRate: { 
      ...
      const userAnalytics = await Analytics.aggregate([
        ...
        },
        {
          $project: {
            ...
            sessionDuration: { 
              $divide: [{ $subtract: ['$lastActivity', '$firstActivity'] }, 60000] 
            }
          }
        },
        { $sort: { totalViews: -1 } },
